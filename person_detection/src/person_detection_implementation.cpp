...
PersonDetectionConfig loadConfiguration(rclcpp_lifecycle::LifecycleNode * node)
{
  ...
  config.track_threshold = dec_common::declareAndGetParameter(
    node, "track_threshold",
    config.track_threshold);
  config.track_buffer =
    dec_common::declareAndGetParameter(node, "track_buffer", config.track_buffer);
  config.match_threshold = dec_common::declareAndGetParameter(
    node, "match_threshold",
    config.match_threshold);
  config.frame_rate = dec_common::declareAndGetParameter(node, "frame_rate", config.frame_rate);
  config.target_classes = dec_common::declareAndGetParameter(
    node, "target_classes",
  ...
  const rclcpp::Logger & logger)
{
  ...
  for (const auto & cls : target_classes) {
    ...
    auto it = std::find_if(
      COCO_CLASSES.begin(), COCO_CLASSES.end(), [&](const std::string & name) {
        std::string n = name;
        std::transform(
          n.begin(), n.end(), n.begin(), [](unsigned char c) {
            return std::tolower(
              c);
          });
        return n == lower;
...
PersonDetectionNode::CallbackReturn PersonDetectionNode::on_configure(
  const rclcpp_lifecycle::State &)
{
  config_ = loadConfiguration(this);

  pub_objects_ =
    create_publisher<dec_interfaces::msg::PersonDetection>("/person_detection/data", 10);
  ...
  if (verbose_mode_) {
    std::string names;
    for (int idx : target_class_indices_) {
      names += COCO_CLASSES[idx] + " ";
    }
...
PersonDetectionNode::CallbackReturn PersonDetectionNode::on_activate(
...
PersonDetectionNode::CallbackReturn PersonDetectionNode::on_deactivate(
  const rclcpp_lifecycle::State & state)
{
  if (vis_timer_) {
    vis_timer_->cancel();
...
PersonDetectionNode::CallbackReturn PersonDetectionNode::on_shutdown(
  const rclcpp_lifecycle::State &)
{
...
void PersonDetectionNode::processImages()
{
  ...
  if (!depth_image_.empty() && !checkCameraResolution(color_image_, depth_image_)) {
    RCLCPP_WARN(
      get_logger(), "%s: Color and depth image resolutions do not match",
      node_name_.c_str());
    return;
  }
...
std::vector<TrackingDatum> PersonDetectionNode::prepareTrackingData(
  const byte_tracker::Detections & tracked)
{
...
cv::Mat PersonDetectionNode::drawTrackedObjects(
...
Yolov11Node::CallbackReturn Yolov11Node::on_configure(const rclcpp_lifecycle::State & state)
{
  ...
  tracker_ = std::make_unique<byte_tracker::ByteTrack>(
    track_thresh_, track_buffer_, match_thresh_,
    frame_rate_);
...
bool Yolov11Node::initModel()
{
  try {
    ...
    try {
      ...
      RCLCPP_WARN(
        get_logger(), "%s: CUDAExecutionProvider unavailable (%s); running on CPU",
    ...
    for (auto & n : input_names_) {
      input_ptrs.push_back(n.c_str());
    }
    std::vector<const char *> output_ptrs;
    for (auto & n : output_names_) {
      output_ptrs.push_back(n.c_str());
    }

    session_->Run(
      Ort::RunOptions{nullptr}, input_ptrs.data(), &input_tensor, 1, output_ptrs.data(),
      output_ptrs.size());
    ...
    RCLCPP_ERROR(
      get_logger(), "%s: Failed to initialize ONNX model: %s", node_name_.c_str(),
      e.what());
    return false;
  }
}
...
Ort::Value Yolov11Node::prepareInput(const cv::Mat & image)
{
  ...
  return Ort::Value::CreateTensor<float>(
    mem_info, chw.data(), chw.size(), shape.data(),
    shape.size());
}

Eigen::Vector4d Yolov11Node::rescaleBox(
  const Eigen::Vector4d & box, double orig_w, double orig_h, double input_w,
  double input_h)
{
...
std::vector<int> Yolov11Node::nms(
...
std::vector<int> Yolov11Node::multiclassNms(
  ...
  double iou_threshold)
{
  ...
  for (int cid : unique_classes) {
    ...
    for (int k : keep) {
      final_keep.push_back(idx[k]);
    }
  }
  return final_keep;
}

bool Yolov11Node::detectObject(
  const cv::Mat & image, std::vector<Eigen::Vector4d> & out_boxes,
  std::vector<float> & out_scores, std::vector<int> & out_class_ids)
{
  ...
  auto outputs = session_->Run(
    Ort::RunOptions{nullptr}, input_ptrs.data(), &input_tensor, 1,
...
void Yolov11Node::processOutput(
  const Ort::Value & output, std::vector<Eigen::Vector4d> & out_boxes,
  std::vector<float> & out_scores, std::vector<int> & out_class_ids)
{
  ...
  for (int64_t i = 0; i < num_preds; ++i) {
    ...
    Eigen::Vector4d rescaled = rescaleBox(
      {cx, cy, w, h}, orig_width_, orig_height_,
...
byte_tracker::Detections Yolov11Node::updateTracker(
  const std::vector<Eigen::Vector4d> & boxes,
