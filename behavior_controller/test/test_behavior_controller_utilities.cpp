...
TEST(Singletons, KnowledgeManagerThrowsBeforeLoad) {
  EXPECT_THROW(
    KnowledgeManager::instance().getUtilityPhrase("1"),
    std::runtime_error);
  EXPECT_THROW(
    KnowledgeManager::instance().getLocationInfo("1"),
    std::runtime_error);
  EXPECT_THROW(
    KnowledgeManager::instance().getTourSpecification(),
    std::runtime_error);
}
...
TEST(Singletons, ConfigManagerAppliesDefaultsForMissingKeys) {
  ...
  EXPECT_EQ(
    config.getEnvironmentKnowledgeBasePath(),
