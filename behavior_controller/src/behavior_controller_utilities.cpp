...
bool ConfigManager::loadFromFile(const std::string & configPath)
{
  try {
    ...
    cultureKnowledgeBasePath = config["culture_knowledge_base"].as<std::string>(
      "cultureKnowledgeBase.yaml");
    environmentKnowledgeBasePath = config["environment_knowledge_base"].as<std::string>(
...
{
  return cultureKnowledgeBasePath;
}
std::string ConfigManager::getEnvironmentKnowledgeBasePath()    const
{
  return environmentKnowledgeBasePath;
}
...
Logger::Logger(std::shared_ptr<rclcpp::Node> node)
: node(node) {}
...
bool ServiceManager::waitForService(
...
TopicMonitor::TopicMonitor(std::shared_ptr<rclcpp::Node> node)
: node(node) {}
...
bool TopicMonitor::waitForTopic(
...
bool TextUtils::containsAnyWord(
...
bool validateEnvironmentKnowledgeBase(const std::string & filePath)
{
  ...
  try {
    ...
    RCLCPP_ERROR(
      logger, "[KB Validation] YAML parse error in '%s': %s",
