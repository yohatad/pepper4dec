...
AnimateBehaviorNode::AnimateBehaviorNode()
: rclcpp_lifecycle::LifecycleNode("animate_behavior")
{
...
AnimateBehaviorNode::CallbackReturn AnimateBehaviorNode::on_configure(
  const rclcpp_lifecycle::State &)
{
  ...
  stop_service_ = create_service<std_srvs::srv::Trigger>(
    ...
    std::bind(
      &AnimateBehaviorNode::stopServiceCallback, this, std::placeholders::_1,
      std::placeholders::_2),
...
AnimateBehaviorNode::CallbackReturn AnimateBehaviorNode::on_activate(
  const rclcpp_lifecycle::State & state)
{
  ...
  joint_sub_ = create_subscription<sensor_msgs::msg::JointState>(
    "/joint_states", 10,
    std::bind(&AnimateBehaviorNode::jointStatesCallback, this, std::placeholders::_1));
  ...
  RCLCPP_INFO(
    get_logger(),
    "activated — animation@%.1fHz, feedback@2Hz | waiting for goals on /animate_behavior",
    update_rate_);
  return CallbackReturn::SUCCESS;
}
...
AnimateBehaviorNode::CallbackReturn AnimateBehaviorNode::on_deactivate(
  const rclcpp_lifecycle::State & state)
{
...
AnimateBehaviorNode::CallbackReturn AnimateBehaviorNode::on_shutdown(
  const rclcpp_lifecycle::State &)
{
  ...
  const rclcpp_action::GoalUUID &, std::shared_ptr<const AnimateBehaviorAction::Goal> goal)
{
  ...
  if (verbose_mode_) {
    RCLCPP_INFO(
      get_logger(), "goal: %s, range=%.2f, duration=%d",
...
void AnimateBehaviorNode::handleAccepted(
...
void AnimateBehaviorNode::sendRgbFade(
...
void AnimateBehaviorNode::cascadeWave()
{
  ...
  double t_hold_end = static_cast<double>(n - 1) * led_white_step_ + led_fade_duration_ +
    led_white_hold_;
  for (size_t i = 0; i < n; ++i) {
