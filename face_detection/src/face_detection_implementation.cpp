...
FaceDetectionConfig loadConfiguration(rclcpp_lifecycle::LifecycleNode * node)
{
  ...
  config.use_compressed = dec_common::declareAndGetParameter(
    node, "use_compressed",
    config.use_compressed);
  config.camera = dec_common::declareAndGetParameter(node, "camera", config.camera);
  config.verbose_mode =
    dec_common::declareAndGetParameter(node, "verbose_mode", config.verbose_mode);
  config.image_timeout = dec_common::declareAndGetParameter(
    node, "image_timeout",
    config.image_timeout);
  config.face_detection_confidence =
    dec_common::declareAndGetParameter(
    node, "face_detection_confidence",
    config.face_detection_confidence);
  config.sixdrepnet_headpose_angle =
    dec_common::declareAndGetParameter(
    node, "sixdrepnet_headpose_angle",
    config.sixdrepnet_headpose_angle);
  config.require_person_detection =
    dec_common::declareAndGetParameter(
    node, "require_person_detection",
    config.require_person_detection);
  config.person_detection_timeout =
    dec_common::declareAndGetParameter(
    node, "person_detection_timeout",
    config.person_detection_timeout);
...
YOLOONNX::YOLOONNX(const std::string & model_path, double class_score_th)
: class_score_th_(class_score_th)
{
  ...
  session_->Run(
    Ort::RunOptions{nullptr}, in_ptrs.data(), &input_tensor, 1,
...
std::pair<std::vector<cv::Rect2d>, std::vector<float>> YOLOONNX::detect(const cv::Mat & image)
{
  ...
  Ort::Value input_tensor = Ort::Value::CreateTensor<float>(
    mem_info, chw.data(),
    chw.size(), shape.data(), shape.size());
  ...
  const cv::Mat & image, const std::vector<float> & raw, int64_t num_boxes, int64_t num_attrs)
{
  ...
  if (num_boxes > 0 && num_attrs >= 7) {
    for (int64_t i = 0; i < num_boxes; ++i) {
      ...
      int x_min =
        static_cast<int>(std::max(static_cast<double>(row[2]), 0.0) * img_w / input_width_);
      int y_min =
        static_cast<int>(std::max(static_cast<double>(row[3]), 0.0) * img_h / input_height_);
      int x_max = static_cast<int>(
        std::min(
          static_cast<double>(row[4]),
          static_cast<double>(input_width_)) * img_w / input_width_);
      int y_max = static_cast<int>(
        std::min(
          static_cast<double>(row[5]),
          static_cast<double>(input_height_)) * img_h / input_height_);
...
FaceDetectionNode::CallbackReturn FaceDetectionNode::on_activate(
  const rclcpp_lifecycle::State & state)
{
  ...
  if (require_person_detection_) {
    ...
    if (verbose_mode_) {
      RCLCPP_INFO(
        get_logger(), "%s: person detection ENABLED — subscribed to /person_detection/data",
        node_name_.c_str());
    }
  } else if (verbose_mode_) {
    RCLCPP_INFO(
      get_logger(), "%s: person detection DISABLED — standalone mode",
      node_name_.c_str());
  }
...
FaceDetectionNode::CallbackReturn FaceDetectionNode::on_deactivate(
  const rclcpp_lifecycle::State & state)
{
  if (vis_timer_) {
    vis_timer_->cancel();
...
SixDRepNet::CallbackReturn SixDRepNet::on_configure(const rclcpp_lifecycle::State & state)
{
  ...
  try {
    ...
    RCLCPP_ERROR(
      get_logger(), "%s: failed to locate model files: %s", node_name_.c_str(),
      e.what());
  ...
  try {
    sixdrepnet_env_ = std::make_unique<Ort::Env>(
      ORT_LOGGING_LEVEL_ERROR,
      "face_detection_sixdrepnet");
    Ort::SessionOptions session_options;
    session_options.SetIntraOpNumThreads(static_cast<int>(std::thread::hardware_concurrency()));
    session_options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_ALL);
    ...
    sixdrepnet_session_ = std::make_unique<Ort::Session>(
      *sixdrepnet_env_, sixdrepnet_model_path.c_str(),
      session_options);

    Ort::AllocatorWithDefaultOptions allocator;
    for (size_t i = 0; i < sixdrepnet_session_->GetInputCount(); ++i) {
      sixdrepnet_input_names_.emplace_back(
        sixdrepnet_session_->GetInputNameAllocated(
          i,
          allocator).get());
    }
    for (size_t i = 0; i < sixdrepnet_session_->GetOutputCount(); ++i) {
      sixdrepnet_output_names_.emplace_back(
        sixdrepnet_session_->GetOutputNameAllocated(
          i,
          allocator).get());
    }
    ...
    Ort::Value input_tensor = Ort::Value::CreateTensor<float>(
      mem_info, dummy.data(), dummy.size(), shape.data(), shape.size());
    std::vector<const char *> in_ptrs;
    for (auto & n : sixdrepnet_input_names_) {
      in_ptrs.push_back(n.c_str());
    }
    std::vector<const char *> out_ptrs;
    for (auto & n : sixdrepnet_output_names_) {
      out_ptrs.push_back(n.c_str());
    }
    sixdrepnet_session_->Run(
      Ort::RunOptions{nullptr}, in_ptrs.data(), &input_tensor, 1, out_ptrs.data(),
      out_ptrs.size());

    if (verbose_mode_) {
      RCLCPP_INFO(
        get_logger(), "%s: SixDRepNet loaded — %s", node_name_.c_str(),
...
void SixDRepNet::drawAxis(
  cv::Mat & img, double yaw, double pitch, double roll, double tdx,
  double tdy, double size)
{
  ...
  cv::line(
    img, origin, cv::Point(static_cast<int>(x1), static_cast<int>(y1)), cv::Scalar(
      0, 0,
      255), 2);
  cv::line(
    img, origin, cv::Point(static_cast<int>(x2), static_cast<int>(y2)), cv::Scalar(
      0, 255,
      0), 2);
  cv::line(
    img, origin, cv::Point(static_cast<int>(x3), static_cast<int>(y3)), cv::Scalar(
      255, 0,
      0), 2);
}

double SixDRepNet::calculateMatchingCost(
  const FaceCandidate & face,
  const PersonCandidate & person) const
{
  if (!(person.x1 <= face.cx && face.cx <= person.x2 && person.y1 <= face.cy &&
    face.cy <= person.y2))
  {
  ...
  return normalized_distance * 2.0 + vertical_penalty * 1.5 + size_penalty * 1.0 + confidence_cost *
         0.5;
}
  ...
  try {
    ...
    for (int f_idx = 0; f_idx < n_faces; ++f_idx) {
      ...
      if (best_person_idx >= 0) {
        ...
        if (verbose_mode_) {
          RCLCPP_INFO(
            get_logger(), "Additional face %d matched to person %s (multiple faces in same box)",
...
float SixDRepNet::getBestDepthEstimate(
...
cv::Mat SixDRepNet::processFrameStandalone(const cv::Mat & cv_image)
{
  ...
  for (size_t i = 0; i < face_boxes.size(); ++i) {
    ...
    if (!(fx1 >= 0 && fx1 < fx2 && fx2 <= img_w && fy1 >= 0 && fy1 < fy2 && fy2 <= img_h)) {
      continue;
    }
  ...
  for (size_t i = 0; i < tracked.xyxy.size(); ++i) {
    ...
    if (face_colors_.find(face_id) == face_colors_.end()) {
      face_colors_[face_id] = generateDarkColor();
    }
    ...
    cv::putText(
      debug_image, mutual_gaze ? "Engaged" : "Not Engaged", cv::Point(fx1 + 10, fy1 - 10),
      cv::FONT_HERSHEY_SIMPLEX, 0.6, face_color, 2);
    cv::putText(
      debug_image, "ID: " + face_id, cv::Point(fx1 + 10, fy1 - 30), cv::FONT_HERSHEY_SIMPLEX, 0.6,
      face_color, 2);
    if (cz > 0.0f) {
      char buf[64];
      std::snprintf(buf, sizeof(buf), "Depth: %.2fm", cz);
      cv::putText(
        debug_image, buf, cv::Point(
          fx1 + 10,
          fy2 + 20), cv::FONT_HERSHEY_SIMPLEX, 0.5, face_color,
        2);
    }
  }
...
cv::Mat SixDRepNet::processFrameWithPersonDetection(const cv::Mat & cv_image)
{
  ...
  double detection_age = 0.0;
  {
    ...
    if (detection_age > person_detection_timeout_) {
      if (verbose_mode_) {
        RCLCPP_WARN(
          get_logger(), "Person detection data is stale (%.2fs old, timeout=%.2fs)",
  ...
  if (verbose_mode_ && n_detections > 0) {
    RCLCPP_INFO(
      get_logger(), "Person detection: %zu persons (age: %.2fs)", n_detections, detection_age);
  }
  ...
  for (size_t i = 0; i < n_detections; ++i) {
    ...
    if (verbose_mode_) {
      RCLCPP_INFO(
        get_logger(), "Person tracking_id=%s: box=(%d,%d,%d,%d), depth=%.2fm",
  ...
  if (persons.empty()) {
    if (verbose_mode_) {
      RCLCPP_INFO(get_logger(), "No valid persons detected (total objects: %zu)", n_detections);
    }
    publishFaceDetection({});
    return debug_image;
  }
  ...
  if (faces.empty()) {
    if (verbose_mode_) {
      RCLCPP_INFO(get_logger(), "No valid faces detected (persons: %zu)", persons.size());
    }
    publishFaceDetection({});
    if (verbose_mode_) {
      for (const auto & person : persons) {
        cv::rectangle(
          debug_image, cv::Point(static_cast<int>(person.x1), static_cast<int>(person.y1)),
          cv::Point(static_cast<int>(person.x2), static_cast<int>(person.y2)),
          cv::Scalar(128, 128, 128), 1);
        cv::putText(
          debug_image, "P:" + person.tracking_id,
  ...
  if (verbose_mode_) {
    RCLCPP_INFO(
      get_logger(), "Hungarian matching: Persons=%zu, Faces=%zu, Matches=%zu",
  ...
  if (matches.empty()) {
    ...
    if (verbose_mode_) {
      RCLCPP_WARN(
        get_logger(), "Faces detected (%zu) but none matched to persons (%zu)", faces.size(),
        persons.size());
      for (const auto & face : faces) {
        cv::rectangle(
          debug_image, cv::Point(static_cast<int>(face.x1), static_cast<int>(face.y1)),
          cv::Point(static_cast<int>(face.x2), static_cast<int>(face.y2)), cv::Scalar(0, 0, 255),
          1);
        char buf[64];
        std::snprintf(buf, sizeof(buf), "Unmatched (score=%.2f)", face.score);
        cv::putText(
          debug_image, buf, cv::Point(static_cast<int>(face.x1), static_cast<int>(face.y1) - 5),
          cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(0, 0, 255), 1);
      }
      for (const auto & person : persons) {
        cv::rectangle(
          debug_image, cv::Point(static_cast<int>(person.x1), static_cast<int>(person.y1)),
          cv::Point(static_cast<int>(person.x2), static_cast<int>(person.y2)), cv::Scalar(
            255, 0,
            0), 2);
        cv::putText(
          debug_image, "Person:" + person.tracking_id,
          cv::Point(static_cast<int>(person.x1) + 5, static_cast<int>(person.y1) - 5),
  ...
  for (const auto & [face_idx, person_idx] : matches) {
    ...
    if (face_colors_.find(base_id) == face_colors_.end()) {
      face_colors_[base_id] = generateDarkColor();
    }
  ...
  if (verbose_mode_) {
    for (const auto & person : persons) {
      cv::Scalar color = person.assigned_faces > 0 ? cv::Scalar(0, 255, 0) : cv::Scalar(
        128, 128,
        128);
      cv::rectangle(
        debug_image, cv::Point(static_cast<int>(person.x1), static_cast<int>(person.y1)),
      ...
      if (person.assigned_faces > 0) {
        label += " (" + std::to_string(person.assigned_faces) + " faces)";
      }
      cv::putText(
        debug_image, label,
        cv::Point(static_cast<int>(person.x1) + 5, static_cast<int>(person.y1) - 5),
