...
AgeGenderDetectionConfig loadAgeGenderConfiguration(rclcpp_lifecycle::LifecycleNode * node)
{
  ...
  try {
    default_model_path =
      ament_index_cpp::get_package_share_directory("face_detection") +
      "/models/face_detection_mivolo_agegender.onnx";
  } catch (const std::exception &) {
    default_model_path = "";
  }

  config.mivolo_model_path = dec_common::declareAndGetParameter(
    node, "mivolo_model_path",
    default_model_path);
  ...
  config.person_topic =
    dec_common::declareAndGetParameter(node, "person_topic", config.person_topic);
  config.image_topic = dec_common::declareAndGetParameter(node, "image_topic", config.image_topic);
  config.output_topic =
    dec_common::declareAndGetParameter(node, "output_topic", config.output_topic);
  ...
  config.min_estimate_interval_sec =
    ...
    node, "min_estimate_interval_sec",
    config.min_estimate_interval_sec);
  config.re_estimate_interval_sec =
    dec_common::declareAndGetParameter(
    node, "re_estimate_interval_sec",
    config.re_estimate_interval_sec);
...
AgeGenderBoundingBox AgeGenderBoundingBox::fromCentroid(
  const geometry_msgs::msg::Point & centroid, float width,
...
void AgeGenderPersonProfile::addEstimate(
  double age_val, const std::string & gender_val,
  double gender_conf)
{
...
std::string AgeGenderPersonProfile::toJson() const
{
  ...
  if (last_person_bbox) {
    char bbox_buf[256];
    std::snprintf(
      bbox_buf, sizeof(bbox_buf), "{\"x1\":%.2f,\"y1\":%.2f,\"x2\":%.2f,\"y2\":%.2f}",
  ...
  return "{\"label_id\":\"" + jsonEscape(label_id) + "\"," + "\"age\":" + age_json + "," +
         ...
         "\"estimation_count\":" + std::to_string(estimation_count) + "," + "\"person_bbox\":" +
         bbox_json + "}";
}
...
MiVOLOONNX::MiVOLOONNX(const std::string & model_path, bool use_cuda)
{
  ...
  Ort::Value input_tensor =
    ...
    mem_info, dummy.data(), dummy.size(), shape.data(),
    shape.size());
...
void MiVOLOONNX::writeCropPlanes(const cv::Mat & crop, float * dst) const
{
  ...
  if (crop.rows == kInputSize && crop.cols == kInputSize) {
    ...
    double r = std::min(
      static_cast<double>(kInputSize) / crop.rows,
      static_cast<double>(kInputSize) / crop.cols);
    ...
    cv::copyMakeBorder(
      resized, letterboxed, top, bottom, left, right, cv::BORDER_CONSTANT, cv::Scalar(
        0, 0,
        0));
  }
...
std::optional<MiVOLOONNX::Estimate> MiVOLOONNX::predict(
  const cv::Mat & face_crop,
  const cv::Mat & body_crop)
{
...
AgeGenderDetectionNode::AgeGenderDetectionNode(const std::string & node_name)
: rclcpp_lifecycle::LifecycleNode(node_name) {}

AgeGenderDetectionNode::~AgeGenderDetectionNode() {cleanup();}

AgeGenderDetectionNode::CallbackReturn AgeGenderDetectionNode::on_configure(
  const rclcpp_lifecycle::State &)
{
  ...
  RCLCPP_INFO(
    get_logger(), "%s: loading MiVOLO ONNX model from %s (device=%s)", get_name(),
  ...
  RCLCPP_INFO(
    ...
    "  Output topic: %s\n"
    "  Max depth: %.2fm",
    get_name(), config_.person_topic.c_str(), config_.face_topic.c_str(),
...
AgeGenderDetectionNode::CallbackReturn AgeGenderDetectionNode::on_activate(
  const rclcpp_lifecycle::State & state)
{
  ...
  person_sub_ = create_subscription<dec_interfaces::msg::PersonDetection>(
    config_.person_topic, sensor_qos,
    std::bind(&AgeGenderDetectionNode::personCallback, this, std::placeholders::_1));
  face_sub_ = create_subscription<dec_interfaces::msg::FaceDetection>(
    config_.face_topic, sensor_qos,
    std::bind(&AgeGenderDetectionNode::faceCallback, this, std::placeholders::_1));
  image_sub_ = create_subscription<sensor_msgs::msg::Image>(
    config_.image_topic, sensor_qos,
    std::bind(&AgeGenderDetectionNode::imageCallback, this, std::placeholders::_1));
  ...
  cleanup_timer_ =
    ...
    std::bind(&AgeGenderDetectionNode::cleanupStaleData, this));
  debug_timer_ =
    create_wall_timer(
    std::chrono::seconds(5),
    std::bind(&AgeGenderDetectionNode::debugStatus, this));

  RCLCPP_INFO(get_logger(), "%s: activated", get_name());
  return CallbackReturn::SUCCESS;
}

AgeGenderDetectionNode::CallbackReturn AgeGenderDetectionNode::on_deactivate(
  const rclcpp_lifecycle::State & state)
{
  if (cleanup_timer_) {
...
AgeGenderDetectionNode::CallbackReturn AgeGenderDetectionNode::on_cleanup(
  const rclcpp_lifecycle::State &)
{
  result_pub_.reset();
  mivolo_.reset();
...
AgeGenderDetectionNode::CallbackReturn AgeGenderDetectionNode::on_shutdown(
  const rclcpp_lifecycle::State &)
{
...
void AgeGenderDetectionNode::personCallback(
  const dec_interfaces::msg::PersonDetection::ConstSharedPtr & msg)
{
  ...
  {
    RCLCPP_WARN(
      get_logger(), "%s: person_callback: inconsistent array lengths, skipping message",
      get_name());
    return;
  }
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      if (is_new) {
        ...
        if (face_it != recent_faces_.end() && face_it->second.mutual_gaze) {
          bool depth_ok = face_it->second.depth <= 0.0 ||
            face_it->second.depth < config_.max_depth_m;
          if (depth_ok) {
            RCLCPP_INFO(
              get_logger(), "%s: new person %s has mutual gaze at %.2fm, triggering estimation",
  ...
  for (const auto & label_id : ids_to_estimate) {
    scheduleEstimation(label_id);
  }
}

void AgeGenderDetectionNode::faceCallback(
  const dec_interfaces::msg::FaceDetection::ConstSharedPtr & msg)
{
  ...
  {
    RCLCPP_WARN(
      get_logger(), "%s: face_callback: inconsistent array lengths, skipping message", get_name());
    return;
  }
  ...
  std::vector<std::string> ids_to_estimate;
  {
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      AgeGenderBoundingBox face_bbox =
        ...
        msg->centroids[i], msg->width[i], msg->height[i],
        mutual_gaze);
...
bool AgeGenderDetectionNode::shouldReEstimate(
  const AgeGenderPersonProfile & profile,
...
void AgeGenderDetectionNode::estimateForPerson(const std::string & label_id)
{
  ...
  // Snapshot bboxes together to ensure they are temporally aligned.
  {
    ...
    if (profile.estimation_count > 0) {
      double elapsed = std::chrono::duration<double>(
        std::chrono::steady_clock::now() - profile.last_updated).count();
  ...
  if (gap > config_.max_cache_age_sec) {
    RCLCPP_WARN(
      get_logger(), "%s: skipping %s: image/bbox time gap %.2fs exceeds %.2fs", get_name(),
  ...
  try {
    ...
    if (result) {
      ...
      RCLCPP_INFO(
        get_logger(), "%s: [%s] age=%.1f (raw=%.1f), gender=%s (%.1f%%)", get_name(),
        label_id.c_str(), result->age, result->age,
        result->gender.c_str(), result->gender_confidence * 100.0);
      publishResult(json_result);
    } else {
      RCLCPP_WARN(
        get_logger(), "%s: inference returned nullopt for %s", get_name(),
        label_id.c_str());
    }
  } catch (const std::exception & e) {
    RCLCPP_ERROR(
      get_logger(), "%s: estimation failed for %s: %s", get_name(),
      label_id.c_str(), e.what());
  }
}

std::optional<cv::Mat> AgeGenderDetectionNode::extractCrop(
  const cv::Mat & image,
  const AgeGenderBoundingBox & bbox) const
{
...
void AgeGenderDetectionNode::debugStatus()
{
  ...
  double image_age;
  {
    ...
    image_age = has_image_ ? std::chrono::duration<double>(
      std::chrono::steady_clock::now() - image_timestamp_).count() : -1.0;
  }
