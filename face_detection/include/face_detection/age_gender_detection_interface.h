...
struct AgeGenderBoundingBox
{
  ...
  static AgeGenderBoundingBox fromCentroid(
    const geometry_msgs::msg::Point & centroid, float width, float height,
...
private:
  ...
  bool shouldReEstimate(
    const AgeGenderPersonProfile & profile,
    std::chrono::steady_clock::time_point now) const;
  std::optional<cv::Mat> extractCrop(
    const cv::Mat & image,
