...
TEST(AgeGenderProfile, MedianIgnoresInsertionOrder) {
  ...
  for (double a : {10.0, 20.0, 30.0, 40.0, 50.0}) {
    ascending.addEstimate(a, "male", 0.9);
  }

  AgeGenderPersonProfile shuffled;
  for (double a : {30.0, 50.0, 10.0, 40.0, 20.0}) {
    shuffled.addEstimate(a, "male", 0.9);
  }
...
TEST(AgeGenderProfileJson, SerializesBoundingBoxWhenPresent) {
  ...
  const std::string json = profile.toJson();
  EXPECT_NE(
    json.find("\"person_bbox\":{\"x1\":80.00,\"y1\":170.00,\"x2\":120.00,\"y2\":230.00}"),
...
TEST(AgeGenderProfileJson, IsAWellFormedObject) {
  ...
  for (size_t i = 0; i < json.size(); ++i) {
    ...
    if (c == '"') {in_string = true;} else if (c == '{') {++depth;} else if (c == '}') {--depth;}
