...
int main(int argc, char ** argv)
{
    ...
    auto node = std::make_shared<OvertAttentionSystem>();
    
    const auto name = node->get_name();
    
    RCLCPP_INFO_STREAM(node->get_logger(),
        name << ": " << SOFTWARE_VERSION << R"(
...
//     // Move the head to the default horizontal looking forward pose before shutting down
