...
std::string getImageTopic(
  const std::string & base_topic, bool use_compressed,
  bool is_depth = false);
...
inline std::pair<double, double> pixelToAngles(
  double u, double v, double fx, double fy, double cx,
  double cy)
{
  double x = (u - cx) / fx;
  double y = (v - cy) / fy;
...
private:
  ...
  void publishVisualization(
    const cv::Mat & bgr, const cv::Mat & saliency,
...
private:
  ...
  double calculateFacePriority(
    const geometry_msgs::msg::Point & centroid, bool mutual_gaze,
    bool is_current_face);
  double calculateIorSuppression(double age_seconds);
  double applyIorFilter(double world_yaw, double world_pitch, double score);
  void cleanupWeakIor();
  void publishHead(
    double yaw, double pitch, double score = 0.0,
  ...
  rclcpp_lifecycle::LifecyclePublisher<naoqi_bridge_msgs::msg::JointAnglesWithSpeed>::SharedPtr
    pub_head_;
