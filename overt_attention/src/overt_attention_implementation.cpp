...
OvertAttentionNode::OvertAttentionNode()
: LifecycleNode("overt_attention")
{
  try {
...
OvertAttentionNode::on_configure(const rclcpp_lifecycle::State &)
{
  ...
  sub_caminfo_ = create_subscription<sensor_msgs::msg::CameraInfo>(
    camera_info_topic, qos_img,
    std::bind(&OvertAttentionNode::onCamInfo, this, std::placeholders::_1));
  sub_joint_states_ = create_subscription<sensor_msgs::msg::JointState>(
    "/joint_states", 10,
    std::bind(&OvertAttentionNode::onJointStates, this, std::placeholders::_1));
  ...
  srv_enable_ = create_service<std_srvs::srv::SetBool>(
    ...
    std::bind(
      &OvertAttentionNode::handleSetEnabled, this, std::placeholders::_1,
      std::placeholders::_2));
  ...
  RCLCPP_INFO(
    get_logger(), "Disable mode: %s (yaw=%.1f\xc2\xb0, pitch=%.1f\xc2\xb0)",
...
void OvertAttentionNode::handleSetEnabled(
...
double OvertAttentionNode::calculateFacePriority(
  ...
  bool mutual_gaze, bool is_current_face)
{
  ...
  double dist_from_center = std::sqrt(
    std::pow(centroid.x - *cx_, 2) + std::pow(
      centroid.y - *cy_,
      2));
...
void OvertAttentionNode::onFaces(const dec_interfaces::msg::FaceDetection::SharedPtr msg)
{
  ...
  for (size_t i = 0; i < msg->centroids.size(); ++i) {
    FaceCandidate cand;
    cand.face_id =
      (i < msg->face_label_id.size()) ? msg->face_label_id[i] : ("unknown_" + std::to_string(i));
    ...
    auto [cam_yaw, cam_pitch] = pixelToAngles(
      cand.centroid.x, cand.centroid.y, *fx_, *fy_, *cx_,
      *cy_);
  ...
  if (is_new_face) {
    ...
    target_pitch_.reset();
    RCLCPP_INFO(
      get_logger(), "Switching to face: %s (engaged=%s, depth=%.2fm, priority=%.2f, reason=%s)",
...
double OvertAttentionNode::applyIorFilter(double world_yaw, double world_pitch, double score)
{
  ...
  for (const auto & loc : visited_locations_) {
    ...
    double dist =
      std::sqrt(std::pow(world_yaw - loc.yaw, 2) + std::pow(world_pitch - loc.pitch, 2));
...
void OvertAttentionNode::onSaliency(const std_msgs::msg::Float32MultiArray::SharedPtr msg)
{
  ...
  if (current_face_id_.has_value()) {
    RCLCPP_INFO(
      get_logger(), "No recent faces, switching to saliency (was tracking: %s)",
  ...
  if (current_saliency_target_.has_value()) {
    ...
    double dist =
      std::sqrt(std::pow(best_yaw - curr_yaw, 2) + std::pow(best_pitch - curr_pitch, 2));
...
void OvertAttentionNode::publishHead(
  double yaw, double pitch, double score,
  const std::string & source, bool force)
{
  ...
  RCLCPP_INFO(
    get_logger(), "[%s] \xe2\x86\x92 yaw=%.1f\xc2\xb0, pitch=%.1f\xc2\xb0, score=%.2f",
