...
: Node("attention_visualization")
{
  ...
  RCLCPP_INFO(
    get_logger(), "Improved visualization ready (Faces w/ Tracking + Engagement + Saliency)");
}
...
void VisualizationNode::onCamInfo(const sensor_msgs::msg::CameraInfo::SharedPtr msg)
{
  if (!fx_.has_value()) {
    ...
    cy_ = msg->k[5];
    RCLCPP_INFO(
      get_logger(), "Camera info: fx=%.1f, fy=%.1f, cx=%.1f, cy=%.1f",
      *fx_, *fy_, *cx_, *cy_);
  }
}
...
void VisualizationNode::onTarget(const geometry_msgs::msg::Vector3::SharedPtr msg)
{
  ...
  if (target_count_ == 1) {
    RCLCPP_INFO(
      get_logger(), "First target received: yaw=%.1f\xc2\xb0, pitch=%.1f\xc2\xb0",
...
void VisualizationNode::processFrame(
  const cv::Mat & frame,
...
void VisualizationNode::drawFace(cv::Mat & vis, const FaceInfo & face, bool is_targeted)
{
  ...
  if (show_depth_ && depth > 0) {
    ...
    cv::putText(
      vis, depth_text.str(), cv::Point(x1, y2 + 25),
      cv::FONT_HERSHEY_SIMPLEX, 0.6, color, 2, cv::LINE_AA);
  }
...
void VisualizationNode::drawSaliencyPeak(cv::Mat & vis, const SaliencyPeak & peak, int rank)
{
  ...
  cv::putText(
    vis, label.str(), cv::Point(u + 25, v + 5 + rank * 20),
...
void VisualizationNode::drawTarget(cv::Mat & vis, int width, int height)
{
  ...
  if (!(u >= 0 && u < width && v >= 0 && v < height)) {
    cv::putText(
      vis, "TARGET OFF-SCREEN", cv::Point(width / 2 - 100, 30),
  ...
  cv::putText(
    vis, label.str(), cv::Point(u + 50, v - 50),
...
void VisualizationNode::drawInfo(cv::Mat & vis)
{
  ...
  for (const auto & item : legend_items) {
    ...
    cv::putText(
      vis, item.label + ": " + item.desc, cv::Point(45, y),
