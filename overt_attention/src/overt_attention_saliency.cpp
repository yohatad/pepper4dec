...
: Node("saliency_node")
{
  ...
  RCLCPP_INFO(
    get_logger(), "Saliency node ready @ %.1f Hz (depth_weighting=%s)",
...
void SaliencyNode::loadParameters()
{
  ...
  image_topic_base_ = selectCameraTopic(
    camera_type_, topics_config_.image.pepper,
    topics_config_.image.realsense);
  depth_topic_base_ = selectCameraTopic(
    camera_type_, topics_config_.depth.pepper,
    topics_config_.depth.realsense);
...
void SaliencyNode::processFrame(
  const cv::Mat & bgr, int width, int height,
...
void SaliencyNode::publishVisualization(
  const cv::Mat & bgr, const cv::Mat & S,
  ...
  const builtin_interfaces::msg::Time & stamp)
{
  ...
  for (size_t idx = 0; idx < peaks.size(); ++idx) {
    ...
    cv::drawMarker(
      vis_color, cv::Point(static_cast<int>(u), static_cast<int>(v)), color,
