...
| What | Name |
|------|------|
| Wheel odom topic (from naoqi_driver2) | `/wheel_odom` |
...
- `bt_navigator.odom_topic: wheel_odom` — BT navigator subscribes to wheel odom topic
...
**`ekf_nav.yaml.yaml`**
- `odom0: /wheel_odom` — EKF input: raw wheel odometry from the driver
