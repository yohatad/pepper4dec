...
std::pair<Eigen::VectorXd, Eigen::MatrixXd> KalmanFilter::initiate(
  const Eigen::Vector4d & measurement) const
{
  ...
  std_dev << 2 * std_weight_position_ * h, 2 * std_weight_position_ * h, 1e-2,
    2 * std_weight_position_ * h,
    10 * std_weight_velocity_ * h, 10 * std_weight_velocity_ * h, 1e-5,
...
{
  double h = mean[3];
  Eigen::Vector4d std_dev(std_weight_position_ * h, std_weight_position_ * h, 1e-1,
  ...
  const Eigen::Vector4d & measurement) const
{
  ...
  Eigen::MatrixXd new_covariance = covariance - kalman_gain * projected_cov *
...
void STrack::activate(
  const KalmanFilter & kf, int frame_id_in, int internal_id,
...
void STrack::reActivate(const STrack & new_track, int frame_id_in)
{
  std::tie(
    mean,
    covariance) = kalman_filter_->update(mean, covariance, tlwhToXyah(new_track.tlwh()));
  tracklet_len = 0;
  state = TrackState::Tracked;
  frame_id = frame_id_in;
...
Eigen::MatrixXd boxIouBatch(
  const std::vector<Eigen::Vector4d> & boxes_true,
...
std::vector<STrack> subTracks(const std::vector<STrack> & a, const std::vector<STrack> & b)
{
  ...
  for (const auto & t : b) {
    ids_b.insert(t.internal_track_id);
  }
  std::vector<STrack> result;
  for (const auto & t : a) {
...
ByteTrack::ByteTrack(
  float track_activation_threshold, int lost_track_buffer, float minimum_matching_threshold,
...
std::vector<STrack> ByteTrack::update(
  ...
  const std::vector<float> & scores)
{
  ...
  for (size_t i = 0; i < dets.size(); ++i) {
    detections.emplace_back(
      STrack::tlbrToTlwh(dets[i]), scores_keep[i],
      minimum_consecutive_frames_);
  }
  ...
  for (int it : assoc1.unmatched_rows) {
    if (strack_pool[it]->state == TrackState::Tracked) {
      r_tracked_stracks.push_back(strack_pool[it]);
    }
  }
