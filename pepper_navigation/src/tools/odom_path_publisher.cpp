...
public:
  ...
  : Node("odom_path_publisher")
  {
    ...
    odom_sub_ = create_subscription<nav_msgs::msg::Odometry>(
      "/pepper_odom_filtered", 10,
      std::bind(&OdomPathPublisher::odomCallback, this, std::placeholders::_1));
    ...
    reset_srv_ = create_service<std_srvs::srv::Empty>(
      "reset_odom_path",
      std::bind(
        &OdomPathPublisher::resetCallback, this, std::placeholders::_1,
        std::placeholders::_2));
...
private:
  ...
  void publishStartMarkers(const geometry_msgs::msg::PoseStamped & ps)
  {
    ...
    auto m =
      makeMarker(ps, 0, visualization_msgs::msg::Marker::SPHERE, 0.2, 0.2, 0.2, 0.0, 1.0, 0.0);
    marker_pub_->publish(m);

    // "START" text label above the sphere
    auto t = makeMarker(
      ps, 1, visualization_msgs::msg::Marker::TEXT_VIEW_FACING, 0.0, 0.0, 0.15,
      1.0, 1.0, 1.0);
  ...
  void publishEndMarker(const geometry_msgs::msg::PoseStamped & ps)
  {
    ...
    auto m = makeMarker(
      ps, 2, visualization_msgs::msg::Marker::SPHERE, 0.15, 0.15, 0.15, 1.0, 1.0,
      0.0);
    marker_pub_->publish(m);
  ...
  void logDeviation(const geometry_msgs::msg::PoseStamped & ps)
  {
    ...
    RCLCPP_INFO(
      get_logger(), "Deviation from start: %.4f m  |  pos=(%.3f, %.3f)  |  path points: %zu",
