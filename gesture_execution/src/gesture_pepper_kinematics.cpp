...
std::array<double, 2> getArmShoulderAngles(int arm, double elbow_x, double elbow_y, double elbow_z)
{
  ...
  double dist_1 = std::sqrt(
    std::pow(pos_1[0] - elbow_x, 2) + std::pow(pos_1[1] - elbow_y, 2) +
    std::pow(pos_1[2] - elbow_z, 2));
  double dist_2 = std::sqrt(
    std::pow(pos_2[0] - elbow_x, 2) + std::pow(pos_2[1] - elbow_y, 2) +
...
std::array<double, 2> getArmElbowAngles(
  ...
  double wrist_x, double wrist_y, double wrist_z)
{
  double elbow_roll = getArmElbowRollAngle(
    arm, shoulder_pitch, shoulder_roll, wrist_x, wrist_y,
    wrist_z);
  double elbow_yaw = getArmElbowYawAngle(
    arm, shoulder_pitch, shoulder_roll, elbow_roll, wrist_x,
    wrist_y, wrist_z);
...
std::array<double, 2> getHeadAngles(double camera_x, double camera_y, double camera_z)
{
  ...
  double head_pitch = std::asin((l_2 - camera_z) / std::sqrt(l_4 * l_4 + l_3 * l_3)) + std::atan(
    l_4 / l_3);
