...
{
  if (arm_name == "RArm") {
    return {"RShoulderPitch", "RShoulderRoll", "RElbowYaw", "RElbowRoll", "RWristYaw"};
  }
  if (arm_name == "LArm") {
    return {"LShoulderPitch", "LShoulderRoll", "LElbowYaw", "LElbowRoll", "LWristYaw"};
  }
  if (arm_name == "Leg") {
  ...
  for (const auto & item : root["gestures"]) {
    ...
    if (data["joints"]) {
      for (const auto & j : data["joints"]) {
        desc.arms.push_back(j.as<std::string>());
      }
    }
    ...
    for (const auto & arm_name : desc.arms) {
      ...
      for (const auto & wp : joint_angles[arm_name]) {
        ...
        for (const auto & v : wp) {
          row.push_back(v.as<double>());
        }
        aw.waypoints.push_back(row);
      }
      if (times && times[arm_name]) {
        for (const auto & t : times[arm_name]) {
          aw.times.push_back(t.as<double>());
        }
      }
...
GestureExecutionNode::GestureExecutionNode()
: rclcpp_lifecycle::LifecycleNode("gesture_action_server")
{
...
GestureExecutionNode::CallbackReturn GestureExecutionNode::on_configure(
  const rclcpp_lifecycle::State &)
{
  ...
  marker_pub_ = create_publisher<visualization_msgs::msg::Marker>(
    "/gesture_execution/visualization", 10);
  ...
  action_server_ = rclcpp_action::create_server<GestureAction>(
    ...
    "/gesture_execution",
    std::bind(
      &GestureExecutionNode::handleGoal, this, std::placeholders::_1,
      std::placeholders::_2),
  ...
  RCLCPP_INFO(
    get_logger(), "GestureExecutionNode configured (verbose_mode: %s)",
...
GestureExecutionNode::CallbackReturn GestureExecutionNode::on_activate(
  const rclcpp_lifecycle::State & state)
{
  ...
  joint_sub_ = create_subscription<sensor_msgs::msg::JointState>(
    topics_.joint_states, 10,
    std::bind(&GestureExecutionNode::jointStatesCallback, this, std::placeholders::_1));
  pose_sub_ = create_subscription<nav_msgs::msg::Odometry>(
    topics_.robot_pose, 10,
    std::bind(&GestureExecutionNode::robotPoseCallback, this, std::placeholders::_1));

  RCLCPP_INFO(
    get_logger(),
    "GestureExecutionNode activated — ready for goals on /gesture_execution");
  return CallbackReturn::SUCCESS;
}
...
GestureExecutionNode::CallbackReturn GestureExecutionNode::on_deactivate(
  const rclcpp_lifecycle::State & state)
{
  lifecycle_active_ = false;
  joint_sub_.reset();
...
GestureExecutionNode::CallbackReturn GestureExecutionNode::on_cleanup(
  const rclcpp_lifecycle::State &)
{
...
GestureExecutionNode::CallbackReturn GestureExecutionNode::on_shutdown(
  const rclcpp_lifecycle::State &)
{
  ...
  const rclcpp_action::GoalUUID &, std::shared_ptr<const GestureAction::Goal> goal)
{
  ...
  RCLCPP_INFO(
    get_logger(), "Accepting goal: type='%s' name='%s'",
    goal->gesture_type.c_str(), goal->gesture_name.c_str());
  return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
}

rclcpp_action::CancelResponse GestureExecutionNode::handleCancel(
  const std::shared_ptr<GoalHandleGesture>)
{
...
void GestureExecutionNode::execute(const std::shared_ptr<GoalHandleGesture> goal_handle)
{
  ...
  gesture_type.erase(
    gesture_type.begin(), std::find_if(
      gesture_type.begin(), gesture_type.end(),
      [](unsigned char c) {return !std::isspace(c);}));
  gesture_type.erase(
    std::find_if(
      gesture_type.rbegin(), gesture_type.rend(),
      [](unsigned char c) {return !std::isspace(c);}).base(), gesture_type.end());
  std::transform(
    gesture_type.begin(), gesture_type.end(), gesture_type.begin(),
  ...
  try {
    success = executeGesture(
      gesture_type, goal->gesture_name, goal->gesture_duration,
...
bool GestureExecutionNode::executeGesture(
...
{
  gesture_duration_ms =
    std::max(MIN_GESTURE_DURATION_MS, std::min(gesture_duration_ms, MAX_GESTURE_DURATION_MS));
...
bool GestureExecutionNode::executeDeicticGesture(
  ...
  int64_t duration_ms)
{
  try {
    ...
    if (verbose_mode_) {
      RCLCPP_INFO(
        get_logger(), "Pointing coordinates: (%.1f, %.1f, %.1f)", pointing_x, pointing_y,
        pointing_z);
    }
    ...
    int pointing_arm =
      (pointing_y <= 0.0) ? pepper_kinematics::RIGHT_ARM : pepper_kinematics::LEFT_ARM;
    double shoulder_x = SHOULDER_OFFSET_X;
    double shoulder_y =
      (pointing_arm == pepper_kinematics::LEFT_ARM) ? SHOULDER_OFFSET_Y : -SHOULDER_OFFSET_Y;
    double shoulder_z = SHOULDER_OFFSET_Z + TORSO_HEIGHT;

    double distance = std::sqrt(
      std::pow(pointing_x - shoulder_x, 2) + std::pow(pointing_y - shoulder_y, 2) +
    ...
    publishDeicticVisualization(
      pointing_x, pointing_y, pointing_z, shoulder_x, shoulder_y,
      shoulder_z, pointing_arm);

    return executePointingMotion(
      pointing_arm, shoulder_pitch, shoulder_roll, duration_ms,
...
bool GestureExecutionNode::checkJointLimits(int arm, double shoulder_pitch, double shoulder_roll)
{
  if (arm == pepper_kinematics::RIGHT_ARM) {
    ...
    {
      RCLCPP_ERROR(
        get_logger(), "Right arm limits exceeded: pitch=%.1f deg, roll=%.1f deg",
    ...
    {
      RCLCPP_ERROR(
        get_logger(), "Left arm limits exceeded: pitch=%.1f deg, roll=%.1f deg",
        pepper_kinematics::radiansToDegrees(shoulder_pitch),
        pepper_kinematics::radiansToDegrees(shoulder_roll));
      return false;
    }
...
bool GestureExecutionNode::executePointingMotion(
  ...
  double pointing_z)
{
  try {
    ...
    auto concatRow = [](std::vector<double> a, const std::vector<double> & b,
    ...
    std::vector<double> point_waypoint = concatRow(
      pointing_angles, head_pointing,
      hand_open_position);
    std::vector<double> return_waypoint = concatRow(home_position, head_home, hand_home_position);

    moveJointsBezier(
      joint_names, {home_waypoint, point_waypoint, return_waypoint},
      duration_sec * 2, true);
...
bool GestureExecutionNode::executeIconicGesture(
    ...
    for (const auto & arm_name : gesture.arms) {
      ...
      if (!arm_times.empty()) {
        ...
        for (size_t i = 0; i < num_waypoints; ++i) {
          arm_times[i] = arm_duration * static_cast<double>(i + 1) /
            static_cast<double>(num_waypoints);
        }
      }
...
bool GestureExecutionNode::executeBowingGesture(int64_t bow_angle, int64_t duration_ms)
{
  try {
    double duration_sec = duration_ms / 1000.0;
    double bow_angle_clamped =
      std::max(MIN_BOW_ANGLE, std::min(static_cast<double>(bow_angle), MAX_BOW_ANGLE));
    ...
    moveJointsBezier(
      joint_names, {home_position, bow_position, home_position}, duration_sec * 2,
      true);
    std::this_thread::sleep_for(std::chrono::duration<double>(duration_sec * 2));
    return true;
  } catch (const std::exception & e) {
...
bool GestureExecutionNode::executeNoddingGesture(int64_t nod_angle, int64_t duration_ms)
{
  try {
    double duration_sec = duration_ms / 1000.0;
    double nod_angle_clamped =
      std::max(MIN_NOD_ANGLE, std::min(static_cast<double>(nod_angle), MAX_NOD_ANGLE));
    ...
    moveJointsBezier(
      joint_names, {home_position, nod_position, home_position}, duration_sec * 2,
      true);
    std::this_thread::sleep_for(std::chrono::duration<double>(duration_sec * 2));
    return true;
  } catch (const std::exception & e) {
...
void GestureExecutionNode::moveJointsBezier(
  ...
  double total_duration, bool use_bezier)
{
  ...
  for (size_t i = 0; i < num_waypoints; ++i) {
    times_list[i] = total_duration * static_cast<double>(i + 1) /
      static_cast<double>(num_waypoints);
  }
...
void GestureExecutionNode::publishJointTrajectory(
  const std::vector<std::string> & joint_names,
  ...
  const std::vector<float> & times, bool use_bezier)
{
  ...
  if (verbose_mode_) {
    ...
    RCLCPP_INFO(
      get_logger(), "Trajectory: %zu joints, %zu waypoints over %.2fs",
      num_joints, num_wp, total_time);
  }
}

void GestureExecutionNode::publishDeicticVisualization(
  double target_x, double target_y, double target_z,
